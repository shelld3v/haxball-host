...
function Pick() {
  ...
  this.updatePicker = function(teamId) {
    ...
    if ( !newPicker ) {
      // Choose a picker from Spectators
      newPicker = getNonHostPlayers().find((player) => player.team == 0);
      if ( !newPicker ) return;
      room.setPlayerTeam(newPicker.id, teamId);
    };
    this.pickers[teamId] = newPicker;
    room.sendChat(`${getTag(picker.name)} đã được chọn làm đội trưởng (người pick) cho ${(teamId == 1) ? "RED" : "BLUE"}`);
  };
  this.getPickers = function() {
    return [this.pickers[0].id, this.pickers[1].id];
  };
  // Pick a player
  this.pick = function(id, teamId) {
    ...
    room.sendAnnouncement(`${pickedPlayer.name} đã được chọn vào ${(teamId == 1) ? "RED" : "BLUE"}`, player.id, GREEN);
    this.unlock();
  };
  ...
  this.forcePick = function(teamId) {
    ...
    if ( this.autopick[teamId] ) {
      ...
      if ( !randomPlayer ) room.startGame();
      try {
...
function pickFunc(id, player) {
  if ( !pick.getPickers().includes(player.id) ) {
    room.sendAnnouncement("Bạn không phải là người pick", player.id, RED);
    return false;
...
function autoPickFunc(value, player) {
  ...
  room.sendAnnouncement(`Đã bật chế độ chọn người chơi tự động, chế độ này sẽ được tắt sau khi trận đấu kết thúc`, null, GREEN);
  return true;
}

function randFunc(value, player) {
  var picker = undefined;
  if ( player.id == redPicker.id ) {
    team = 1;
  } else if ( player.id == bluePicker.id ) {
    team = 2;
  }
  if ( !team ) {
}
...
function warnPicker(teamId) {
  let picker = ( teamId == 1 ) ? redPicker : bluePicker;
  room.sendAnnouncement(`${getTag(picker.name)} Đội bạn đang thiếu người, dùng !pick hoặc !rand để bổ sung thêm`, picker.id, YELLOW, "bold", 2);
}

async function pickPlayers() {
  pick.forcePick(1); // Start picking from RED
  pick.lock(1);
}
