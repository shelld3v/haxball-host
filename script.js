...
const MAX_SIZE_ADJUSTMENT_RATIO = 0.2;
const SAVE_RECORDINGS = true;
const MAX_WARNINGS_PER_PLAYER = 10;
...
const TEAM_COLORS = [
    [
        [60, 0xFFCC00, [0xE83030]],
        [60, 0xFFCC00, [0x004170]]
    ],
    [
        [60, 0xFFFFFF, [0xFF4A4A]],
        [60, 0xFFFFFF, [0x5ECFFF]]
    ],
    [
        [60, 0xFFFFFF, [0xD60419]],
        [60, 0xFFFFFF, [0x0099FF]]
    ],
    [
        [45, 0x000000, [0xFF2400, 0xFFFF00]],
        [45, 0x000000, [0x1F51FF, 0xFFFFFF]]
    ],
    [
        [0, 0xF7FFF2, [0xE00202, 0xB00101, 0x800000]],
        [0, 0xF7FFF2, [0x00F7FF, 0x00D1D1, 0x00A7AD]]
    ],
    [
        [90, 0xF7FFF2, [0xFF2121, 0xFF5757, 0xFC9595]],
        [90, 0xF7FFF2, [0x00C3FF, 0x45E0FF, 0xB5F5FC]]
    ],
    [
        [45, 0xFFFFFF, [0x000000, 0xFF0000, 0x000000]],
        [45, 0x808080, [0xFFFFFF, 0x0096FF, 0xFFFFFF]]
    ],
    [
        [45, 0xFFFFFF, [0xD60000, 0x000000, 0xD60000]],
        [45, 0xFFFFFF, [0x0058A3, 0x000000, 0x0058A3]]
    ],
    [
        [-45, 0xFFCC00, [0xD10000, 0x8C0000, 0xD10000]],
        [-45, 0xFFCC00, [0x00DDFF, 0x87E3FF, 0x00DDFF]]
    ],
    [
        [232, 0xFFFFFF, [0xFFCCFA, 0xFF99DD, 0xFF6176]],
        [129, 0xFFFFFF, [0x4D39CC, 0x7A70FF, 0x7DB1FF]]
    ],
];
...
const DISCORD_LINK = "https://discord.gg/P8zpCMxYrM"; // Discord link for the room
...
class Penalty {
    constructor() {
        ...
        this.results = [
            [],
            []
        ]; // Results of taken penalties (first for RED, second for BLUE)
    }
    getTurn() {
...
function getPlayerStats() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        playerList.push(new PlayerReport({
...
async function randomAnnouncement() {
    ...
    default: // Send a random quote
        try {
            (quotes.length == 0) && await fetch("https://api.quotable.io/quotes/random?limit=50", {
                    method: "GET"
                }) // Fetch new quotes
...
async function sendWebhook(title, content, fields, attachments) {
    ...
    for (const attachment of attachments) {
        ...
        form.append(null, new File([attachment[1]], attachment[0], {
            "type": "text/plain"
        }));
        await fetch(DISCORD_WEBHOOK, {
            method: "POST",
...
function resizePlayer(id) {
    ...
    room.setPlayerDiscProperties(id, {
        radius: (+playerRadius) + (+setting.sizeAdjustment)
    });
}
...
async function celebrationEffect(player, hasScored) {
    ...
    case 2:
        ...
        for (let i = 1; i <= 5; i += 1) {
            await room.setPlayerDiscProperties(player.id, {
                radius: stadium.playerRadius - stadium.playerRadius * (i % 2) / 2
            });
            await new Promise(r => setTimeout(r, 500));
        };
...
function discordFunc(value, player) {
    if (!DISCORD_LINK) {
        room.sendAnnouncement(`Discord chính thức của De Paul: https://discord.gg/P8zpCMxYrM`, player.id, YELLOW, "normal", 0);
        return false;
    };
...
function adjustSizeFunc(value, player) {
    if (!value || isNaN(value)) {
        room.sendAnnouncement("Vui lòng cung cấp số đơn vị muốn thay đổi, dùng 0 để chỉnh lại về bình thường ( chỉ có thể giảm kích thước, VD: !adjustsize -2)", player.id, RED);
        return false;
    };
    if (value > 0) {
    ...
    (playerDiscProperties !== null) && room.setPlayerDiscProperties(player.id, {
        radius: playerDiscProperties.radius + gap
    });
...
function reportStats() {
    ...
    let playerStats = [
        ["Người chơi                       ", "Đội ", "Bàn", "Kiến tạo", "Phản lưới", "Đường chuyền", "Sút trúng đích", "Chặn cú sút", "Nỗ lực tạo ra bàn thắng phản lưới", "Sai lầm dẫn đến bàn thua", "Penalty thành công", "Penalty không thành công", "Chạm bóng"]
    ];
...
[2;34m${contributions[1].join("\n")}\`\`\``;
    ...
    sendWebhook(`🌟 ${scoreline}`, discordMsg, discordFields, [
        [new Date().toString().slice(0, 21).replace(":", "h") + ".hbr2", room.stopRecording()],
        ["players_report.txt", playerStats.map(arr => arr.join(" | ")).join("\n")]
    ]);
}
...
async function takePenalty() {
    ...
    room.startGame();
    room.setPlayerDiscProperties(penaltyTaker, {
        invMass: 9999
    }); // Prevent penalty taker from mis-kicking the ball (in some penalty stadiums the ball's invMass is bigger than players')
